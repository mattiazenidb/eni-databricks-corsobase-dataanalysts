...
-- MAGIC %python
-- MAGIC
-- MAGIC catalog = 'eni_databricks_corsobase_dataanalysts'
-- MAGIC schema = json.loads(dbutils.notebook.entry_point.getDbutils().notebook().getContext().safeToJson())["attributes"]["user"].split('@')[0].replace('.', '_')
...
SHOW GRANT ON ${catalog}.${schema}.turbine_training_dataset
...
GRANT SELECT ON TABLE ${catalog}.${schema}.turbine_training_dataset TO `account users`;
